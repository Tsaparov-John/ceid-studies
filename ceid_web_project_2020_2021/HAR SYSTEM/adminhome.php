...
<div class="row">
                 <div class="col-md-1" style="position:fixed; top:10px; transform:scale(0.8); font-size:8px;">
            ...
            <div class="col-md-1" style="position:fixed;  transform:scale(0.8); margin-left:10%; font-size:8px;" >
...
<script>
 var randomColor = function(i) {//regular on takes opacity
//clr=["#083b66","#4d4581","#8c478c","#c64782","#f15568","#ff7842","#ffa600"]
clr=["#1761a0","#6c62b2","#ae5db0","#e45799","#ff5f74","#ff7d47","#ffa600"];
return clr[i];
    // return 'rgba(' + Math.round(Math.random()*255) + ',' + Math.round(Math.random()*255) + ',' + 200 + ',' + (opacity || '.3') + ')';
     };
                    ...
                    $( "#BringChoice" ).click(function() {//to moandik;o koympi
                                ...
                                $.ajax(
                                {
                                    ...
                                    success: function(dataResult){
                                                                   ...
                                                                   dataTrace=[];
                                                                    for(contType in dr )
                                                                    {
                                                                        trace={histfunc:'count',nbinsx:10,x:dr[contType] ,type:"histogram"}
                                                                        dataTrace.push(trace);
                                                                    }
                                                                    console.log(dataTrace)
                                                                    layout={barmode:"stack"};
                                                                    Plotly.newPlot('myhisto',dataTrace,layout);
                                                                    }
                                ...
                                $.ajax(
                                {
                                    ...
                                    success: function(dataResult){
                                                           ...
                                                           console.log(document.getElementById('mydoughnut2'))
                                                            var mydoughnut = new Chart(ctx, {
                                                            type: 'doughnut',
                                                            data: {
                                    ...
                                    success: function(dataResult){
                                                                    ...
                                                                    selectedDay.forEach(
                                                                        function(dayKey){
                                                                            ...
                                                                            var times=[];
                                                                            //dataArray.length=24;
                                                                            // console.log("puting the indices")
                                                                            for(hourKey in dr[dayKey])
                                                                            {   
                                                                                H=Date.parse("01 01 1970 "+hourKey);
                                                                                indH.push(H);
                                                                                times[H]=dr[dayKey][hourKey].avg;
                                                                            }
                                                                            indH.sort ( (a ,b)=> a-b);
                                                                            for(ind in indH)
                                                                            {
                                                                                hourKey=indH[ind];
                                                                                dt=new Date(hourKey);
                                                                                point={x:moment(dt),y:times[hourKey]};
                                                                    ...
                                                                    $.each(dataSet, function(i, dataset) {
                                                                        ...
                                                                        dataset.borderColor = randomColor(i);
                                                                        dataset.backgroundColor = randomColor(i);
                                                                        dataset.pointBorderColor = randomColor(i);
                                                                        dataset.pointBackgroundColor = randomColor(i);
...
<br>
        ...
        <br>
        <div id="myBarChart" class="pt-3 pb-5"></div>   
        <div id="myhisto"></div> 
...
<br>
        <h2 class="p-5">Statistics on Map</h2>
        ...
        <script>
                        ...
                        options: {
                            ...
                            var homeIcon = new LeafIcon({iconUrl: 'img/home.png'});
                            var serverIcon = new LeafIcon({iconUrl: 'img/server.png'});
                    ...
                    ajax.onreadystatechange =function()
					{
						...
						{
                            //converting json back to array
                            var  LocRet=JSON.parse(this.responseText);
                            console.log("4");
                            console.log(LocRet)
                            for(ind in LocRet )
                            {
                                L.marker([LocRet[ind].ULat,LocRet[ind].ULong] , {icon: homeIcon}).addTo(map).bindPopup("I am a home. From DB");
                                L.marker([LocRet[ind].SLat,LocRet[ind].SLong], {icon: serverIcon}).addTo(map).bindPopup("I am a server.From DB");
                                var pointH = new L.LatLng(LocRet[ind].ULat,LocRet[ind].ULong);
                                var pointS = new L.LatLng(LocRet[ind].SLat,LocRet[ind].SLong);
                                var firstpolyline = new L.Polyline([pointH, pointS], {
                                color: 'blue',
                                weight: 10*LocRet[ind].factor,
                                opacity: 0.8,
                                smoothFactor: 0
                            });
